...
class BaseFilter : public QObject
{
  ...
  public:
	...
	**/
	...
	const FilterDescription& getDescription() const
	{
		return m_desc;
	}

	//! Sets whether to show a progressbar while computing or not
	inline void setShowProgressBar(bool status)
	{
	...
	**/
	...
	inline void setMainAppInterface(ccMainAppInterface* app)
	{
		m_app = app;
	}

	//! Returns associated CC application interface for accessing to some of mainWindow methods
	ccMainAppInterface* getMainAppInterface()
	{
		return m_app;
	}
  ...
  protected:
	...
	**/
	virtual int getParametersFromDialog()
	{
		return 1;
	}
