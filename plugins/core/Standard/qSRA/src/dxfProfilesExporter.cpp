		...
		for (unsigned angleStep = 0; angleStep < angularStepCount; ++angleStep)
		{
			...
			// write corresponding polyline
			{
				...
				for (size_t i = 0; i < polySteps.size(); ++i)
				{
					...
					dxf.writeVertex(*dw, DL_VertexData(x0 + (step.radius_th + step.deviation * params.devMagnifyCoef - xMin) * scale, y0 + (step.height - yMin) * scale, 0.0));
				}
		...
		for (unsigned heightStep = 0; heightStep < heightStepCount; ++heightStep)
		{
			...
			// write profile polyline
			{
				...
				for (size_t i = 0; i < polySteps.size(); ++i)
				{
					...
					dxf.writeVertex(*dw, DL_VertexData(pageShift.x - (cwSign * radius * sin(step.angle_rad)) * scale, pageShift.y - (radius * cos(step.angle_rad)) * scale, 0.0));
				}
